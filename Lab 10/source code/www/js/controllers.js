...
.controller('DashCtrl', function($scope,$http, $httpParamSerializerJQLike) {
    ...
    $scope.validate = function(username, password){
        ...
        $http({
            method: 'GET',
            url: 'https://api.mongolab.com/api/1/databases/liuyunl/collections/cs5551lab?q={"name": "' + username + '", "psd": "' + password + '"}&apiKey=RYFE7p2AN7-aIFmOfwVGgyXfe2Z1NsqT',           
...
.controller('Upd', function($scope,$http, $httpParamSerializerJQLike) { 
    ...
    $scope.update = function(NewPassword){
        ...
        $http({
            method: 'PUT',
            url: 'https://api.mongolab.com/api/1/databases/liuyunl/collections/cs5551lab/'+ sessionStorage.getItem("userID")+'?apiKey=RYFE7p2AN7-aIFmOfwVGgyXfe2Z1NsqT',
...
.controller('RegistCtrl', function($scope, $http, $httpParamSerializerJQLike){
    ...
    $scope.register = function(email, username, password) {
        ...
        $http({
            method:'GET',
            url:'https://api.mongolab.com/api/1/databases/liuyunl/collections/cs5551lab?q={"name": "' + username + '"}&apiKey=RYFE7p2AN7-aIFmOfwVGgyXfe2Z1NsqT',
        }).success(function(data){
            console.log(data);
            if (data!=""){
                ...
                $http({
                    method: 'POST',
                    url : 'https://api.mongolab.com/api/1/databases/liuyunl/collections/cs5551lab?apiKey=RYFE7p2AN7-aIFmOfwVGgyXfe2Z1NsqT',
                    data: JSON.stringify({
                        e_mail:email,
                        name: username,
                        psd: password,
